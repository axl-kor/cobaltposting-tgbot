...
class MessageMiddleware(cog.BaseMiddleware):
    def __init__(self, bot: cog.Bot):
        self.bot = bot
        # print("messageMidldeware initialized")
            ...
            if type(event) == cog.CallbackQuery:
                ...
                if "message is not modified" in str(exc).lower():
                    return await cq.answer(
                        "Message text is up to date.", show_alert=True
                    )
...
class RequestLogging(cog.BaseRequestMiddleware):
    def __init__(self, bot: cog.Bot):
        ...
        self.lastLog = self.bot.sh.nowdt()
...
class CastMiddleware(cog.BaseMiddleware):
    ...
    async def __call__(self, handler, event, data):
        if isinstance(event, cog.aiotypes.Message):
            event = cog.Message(
                bot=self.bot, **event.__dict__
            )  # .model_validate(event.model_dump()
            # ).as_(self.bot)
            # self.bot.log(event, type="debug")
